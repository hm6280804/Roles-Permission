                       ...
                       @foreach ($roles as $role)
                        <tr class="border-b">
                                ...
                                <td class="px-6 py-5 text-center">
                                    @can('edit roles')
                                    <a href="{{ route('roles.edit', $role->id) }}" class="bg-slate-700 text-sm rounded-md text-black px-3 py-2 hover:bg-slate-600 mr-2">
                                        Edit
                                    </a> 
                                    @endcan
                                    
                                    @can('delete roles')
                                    <a href="javascript:void(0);" onclick="deleteRole({{ $role->id }})" class="bg-red-700 text-sm rounded-md text-black px-3 py-2 hover:bg-red-600 ml-2">
                                        Delete 
                                    </a> 
                                    @endcan
